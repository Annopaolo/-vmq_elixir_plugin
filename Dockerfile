...
# Cache elixir deps
ADD mix.exs mix.lock vmq_elixir_plugin/
RUN cd vmq_elixir_plugin && \
  mix do deps.get, deps.compile && \
  cd ..

# Add all the rest
ADD . vmq_elixir_plugin/
...
COPY --from=builder /build/vmq_elixir_plugin/_build/$BUILD_ENV/rel/vmq_elixir_plugin /etc/vmq_elixir_plugin
# Copy the schema over
# COPY --from=builder /build/vmq_elixir_plugin/priv/vmq_elixir_plugin.schema /vernemq/share/schema/vmq_elixir_plugin.schema
